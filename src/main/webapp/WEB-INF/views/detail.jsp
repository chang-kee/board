...
<script>
	...
	$(document).ready(function(){
		...
		// 댓글 목록 불어오는 replylist()함수 호출
		replylist();
		
		// 댓글 쓰기를 클릭하면 실행 : 이벤트 발생
		//$("#btnReplySave").click(function(){ });
		//$("#btnReplySave").on('click' ,function(){ }  // 'click' -> 클릭이벤트
		$(document).on('click', '#btnReplySave' ,function(){   // 'click' -> 클릭이벤트
			...
			// 전송방식(POST/GET), 주소(매핑주소), 데이터, 데이터타입(JSON) => 성공 또는 실패
			$.ajax({
				type : "POST",
				url : url,
				data : paramData,
	...
	function replylist(){
	   ...
	   function fn_updateReply(reno, rewriter){
		   ...
		   url = "${pageContext.request.contextPath}/board/replyupdate2"; //ajax수정
		   // paramData
		   var paramData = {
            	"reno" : reno,
            	"rewriter" : rewriter,
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	<div class="box-header with-border">
		...
		<a href="/board/logout" onclick="alert('로그아웃 되었습니다.')"> <h3 class="box-title">로그아웃</h3></a>
		</c:if>
	</div>
	<div class="box">
			...
			<table>
				<tr>
					<td rowspan="2" width="70%">
						<textarea name="rememo" id="rememo" class="form-control" placeholder="내용을 입력하세요"></textarea>
					</td>
					<td>
						<input type="text" name="rewriter" id="rewriter" placeholder="댓글작성자" />
					</td>			
				</tr>
				<tr>
